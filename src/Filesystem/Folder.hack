...
final class Folder extends Node {
  ...
  public async function list<<<__Enforceable>> reify T as Node>(
    ...
    foreach ($iterator as $node) {
      if (
        $node->isDir() &&
        (T::class === Node::class || T::class === Folder::class)
      ) {
        $contents[] = new Folder($node->getPathname()) as T;
      } else if (
        $node->isFile() &&
        (T::class === Node::class || T::class === File::class)
      ) {
        $contents[] = new File($node->getPathname()) as T;
      }
  ...
  public async function move(
    ...
    return await parent::move($target->toString(), $overwrite);
  }
  ...
  public async function mkdir(
    ...
    $folder = new Folder($path->toString());
  ...
  public async function read<<<__Enforceable>> reify T as Node>(
    ...
    $node = Node::load($path->toString());
    if (
      (T::class === File::class && $node is Folder) ||
      (T::class === Folder::class && $node is File)
    ) {
      throw new Exception\InvalidPathException(Str\format(
        'Invalid %s path (%s), %s are not allowed.',
        $node is Folder ? 'file' : 'folder',
        $path->toString(),
