...
abstract class Node {
   ...
   */
  final public static function load(string $path): Node {
    ...
    if ($path->isFolder()) {
      return new Folder($path->toString());
    }
  ...
  public function parent(): ?Folder {
    ...
    if ($folder->toString() !== '.' && $folder->toString() !== '/') {
      $this->parent = new Folder($folder->toString());
    }
  ...
  public async function rename(
    ...
    if ($target->exists()) {
      if ($overwrite) {
        ...
        throw new Exception\ExistingNodeException(Str\format(
          'Cannot rename %s as the target (%s) already exists.',
          $this is Folder ? 'folder' : 'file',
          $target->toString(),
        ));
      }
   ...
   */
  private function normalizePath(string $path): Path {
    $path = Path::create($path);
