...
final class UriMarshaler {
  ...
  private function marshalHostFromHeader(string $host): string {
    ...
    // use Regex\replace() instead of Regex\match() to prevent DoS attacks with long host names
    if (
      '' !== $host &&
  ...
  /**
   * Marshal the host and port from HTTP headers and/or the HHVM environment.
   *
